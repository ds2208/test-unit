...
if [ $MAKE_BACKUP -eq 1 ]; then
    ...
    docker exec -i -u root ${COMPOSE_PROJECT_NAME}_db mysqldump --no-create-info -u root --password=$MYSQL_ROOT_PASSWORD $MYSQL_DATABASE --ignore-table=$MYSQL_DATABASE.languages --ignore-table=$MYSQL_DATABASE.sys_currencies --ignore-table=$MYSQL_DATABASE.sys_countries --ignore-table=$MYSQL_DATABASE.migrations > $backup;
fi
